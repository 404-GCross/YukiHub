...
/**
 * Receives yukihub://hikarinagi/callback from Hikarinagi ID (standard OIDC + PKCE S256)
 * and converts it into a normal YukiHub session via the YukiHub backend proxy.
 * This is an additive third-party quick-login path; the existing email/KUN flows are untouched.
 */
public class HikarinagiOAuthCallbackActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        status.setText("正在完成 Hikarinagi 快捷登录...");
    ...
    private void handleCallback(Uri uri) {
        ...
        if (error != null && !error.trim().isEmpty()) {
            fail("Hikarinagi 登录取消或失败：" + error);
            return;
        }
        String code = uri.getQueryParameter("code");
        String state = uri.getQueryParameter("state");
        String expectedState = prefs == null ? "" : prefs.getString(KEY_HIKARINAGI_OAUTH_STATE, "");
        String verifier = prefs == null ? "" : prefs.getString(KEY_HIKARINAGI_OAUTH_CODE_VERIFIER, "");
        String nonce = prefs == null ? "" : prefs.getString(KEY_HIKARINAGI_OAUTH_NONCE, "");
        long startedAt = prefs == null ? 0L : prefs.getLong(KEY_HIKARINAGI_OAUTH_STARTED_AT, 0L);
        ...
        new Thread(() -> exchangeCode(code.trim(), verifier.trim(), nonce == null ? "" : nonce.trim())).start();
    }

    private void exchangeCode(String code, String codeVerifier, String nonce) {
        try {
            JSONObject body = new JSONObject();
            body.put("code", code);
            body.put("codeVerifier", codeVerifier);
            body.put("nonce", nonce);
            body.put("redirectUri", "yukihub://hikarinagi/callback");

            String mode = prefs == null ? "" : prefs.getString(KEY_HIKARINAGI_OAUTH_MODE, "");
            if (HIKARINAGI_OAUTH_MODE_BIND.equals(mode)) {
                String accessToken = prefs == null ? "" : prefs.getString(KEY_AUTH_ACCESS_TOKEN, "");
                JSONObject resp = postJson(AUTH_BASE_URL + "/auth/hikarinagi/bind", body, accessToken);
                JSONObject user = resp.optJSONObject("user");
                if (user != null) {
                    prefs.edit()
                            .putString(KEY_AUTH_EMAIL, firstString(user, "email"))
                            .putBoolean(KEY_HIKARINAGI_BOUND, true)
                            .apply();
                } else {
                    prefs.edit().putBoolean(KEY_HIKARINAGI_BOUND, true).apply();
                }
                clearPendingOAuth();
                runOnUiThread(() -> openProfile("Hikarinagi 账号绑定成功"));
            } else {
                JSONObject resp = postJson(AUTH_BASE_URL + "/auth/hikarinagi/android_callback", body, null);
                saveSession(resp);
                clearPendingOAuth();
                runOnUiThread(() -> openProfile("Hikarinagi 快捷登录成功"));
            }
        } catch (Throwable t) {
            String mode = prefs == null ? "" : prefs.getString(KEY_HIKARINAGI_OAUTH_MODE, "");
            runOnUiThread(() -> fail((HIKARINAGI_OAUTH_MODE_BIND.equals(mode) ? "Hikarinagi 账号绑定失败：" : "Hikarinagi 快捷登录失败：") + readableError(t)));
        }
    }
    ...
    private void saveSession(JSONObject resp) throws Exception {
        ...
        prefs.edit()
                ...
                .putBoolean(KEY_HIKARINAGI_BOUND, user == null || user.optBoolean("hikarinagiBound", true))
    ...
    private void clearPendingOAuth() {
        ...
        prefs.edit()
                .remove(KEY_HIKARINAGI_OAUTH_STATE)
                .remove(KEY_HIKARINAGI_OAUTH_CODE_VERIFIER)
                .remove(KEY_HIKARINAGI_OAUTH_NONCE)
                .remove(KEY_HIKARINAGI_OAUTH_STARTED_AT)
                .remove(KEY_HIKARINAGI_OAUTH_MODE)
                .apply();
    }
